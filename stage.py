...
# The server connects to a database and performs CRUD operations to modify the database.
# OAuth through Google is used to create user accounts and restrict access to CRUD operations.


# Import objects to connect to process requests and send response content
...
# Import database objects and create engine
...
# Use the one-time use code obtained from the client to request an authorization token.
...
def gconnect():
	...
	# a 'credentials' object which will have the access token as one of it's properties.
	code = request.data
	try:
	...
	# Verify that the token received from Google is valid by sending a request to Google's verification API
	...
	# Confirm that the Google User ID on the verfied token matches the Google User Id 
	# on the code-exchanged token.
	...
	# If not, request user information from Google API and store in the loging session object.
	...
	# Use the user's email as a unique identifier to search the database for a user id.
...
def gdisconnect():
	...
	# If the request is successful, delete the user's information from the session object.
	h = httplib2.Http()
...
# State is stored in the login session to protect against cross-site reference forgery.
@app.route('/login')
def showlogin():
...
def showAlbum(band_id,alb_id):
	...
	if 'username' in login_session:
		...
		return render_template('public`showAlbum.html',b=band,a=album,creator=creator)
...
def editBand(id):
	...
	if login_session['user_id'] != editBand.user_id:
		response = make_response(json.dumps('User not authorized to edit this band.'),401)
		response.headers['Content-Type'] = 'application/json'
		return response
	if request.method == 'POST':
...
def deleteBand(id):
	...
	if login_session['user_id'] != delBand.user_id:
		response = make_response(json.dumps('User not authorized to delete this band.'),401)
		response.headers['Content-Type'] = 'application/json'
		return response
	if request.method == 'POST':
...
def editAlbum(band_id,alb_id):
	...
	if login_session['user_id'] != editAlbum.user_id:
		response = make_response(json.dumps('User not authorized to edit this album.'),401)
		response.headers['Content-Type'] = 'application/json'
		return response
	if request.method == 'POST':
		name = request.form.get('name')
...
def deleteAlbum(band_id,alb_id):
	...
	if login_session['user_id'] != delAlbum.user_id:
		response = make_response(json.dumps('User not authorized to delete this album.'),401)
		response.headers['Content-Type'] = 'application/json'
		return response
	if request.method == 'POST':
